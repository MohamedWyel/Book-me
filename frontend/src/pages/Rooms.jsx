...
export default function Rooms({darkMode}) {
    ...
    const fetchRooms = async () => {
        try {
            const response = await fetch(`http://localhost:5000/api/room/hotel/${id}`);
  ...
  return (
    ...
    <h2 className="text-center fw-bold mb-4 text-warning">Available Rooms in {hotelName}</h2>
    ...
    {rooms.length > 0 ? (
        ...
        <div key={room._id} className="col-md-4 mb-4">
            ...
            <button onClick={()=>{}} className="btn btn-outline-success text-center" style={{width:"45%" ,marginBottom:20}} >Book Now</button>
            </div>
            </div>
        </div>
    ))
